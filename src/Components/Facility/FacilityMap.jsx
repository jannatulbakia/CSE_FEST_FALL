...
const boatIcon = new L.Icon({
  iconUrl: "https://cdn-icons-png.flaticon.com/512/2038/2038969.png",
  iconSize: [28, 28],
});
const chwIcon = new L.Icon({
  ...
  return (
    <MapContainer center={center} zoom={12} style={{ height: "100%", width: "100%" }}>
      ...
      {safeFacilities.map((f) => {
        ...
        return (
          <Marker key={f.id || Math.random()} position={position} icon={icon}>
            <Popup>
              <b>{f.name}</b>
              <br />
              ধরন: {f.type}
              <br />
              উপজেলা: {f.upazila}, ইউনিয়ন: {f.union}
              <br />
              খোলার সময়: {f.open}
              <br />
              {typeof f.distance === "number" && (
                <span>দূরত্ব: {(f.distance/1000).toFixed(2)} km</span>
              )}
              <br />
